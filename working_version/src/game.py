...
class Game:
    ...
    def winner(self) -> Union[None, SquareState]:
        """Check if the game has been won. If so, return the winner. Otherwise, return None."""
        for line in self.board.all_lines:
            if line[0].state == line[1].state == line[2].state != SquareState.EMPTY:
                return line[0].state
        return None
